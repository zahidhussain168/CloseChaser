...
function sanitize(s: string): string {
  ...
  return s.replace(new RegExp("\\s*" + cls + "\\s*", "g"), ", ").replace(new RegExp(cls, "g"), "-").trim();
}

const SYSTEM =
  "You write email templates for RuledOff, a tool solo bookkeepers use to chase their own clients for the documents, receipts, and answers needed to close the books each month. You output ONLY valid JSON. No preamble, no markdown, no code fences.";
